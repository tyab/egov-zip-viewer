...
modeSelect?.addEventListener('change', () => activeIndex >= 0 && selectXml(activeIndex));
xslSelect?.addEventListener('change', () => activeIndex >= 0 && selectXml(activeIndex));
compatSelect?.addEventListener('change', () => activeIndex >= 0 && selectXml(activeIndex));
...
async function loadFile(file) {
  try {
    ...
    if (file.name.toLowerCase().endsWith('.zip')) {
      if (!window.JSZip) throw new Error('JSZipを読み込めません。インターネット接続または同梱版を確認してください。');
      const zip = await JSZip.loadAsync(file);
      const entries = Object.values(zip.files).filter(e => !e.dir);
      for (const e of entries) {
        const lower = e.name.toLowerCase();
        const rec = { name: e.name, lower, zipEntry: e, text: null, blobUrl: null };
        if (isTextLike(lower)) rec.text = decodeTextBuffer(await e.async('arraybuffer'), e.name);
        allEntries.set(normalizePath(e.name), rec);
      }
      xmlFiles = [...allEntries.values()].filter(e => e.lower.endsWith('.xml'));
      xslFiles = [...allEntries.values()].filter(e => e.lower.endsWith('.xsl') || e.lower.endsWith('.xslt'));
      displayFiles = [...allEntries.values()].filter(e => !isHiddenSystemFile(e.lower));
      if (!displayFiles.length) throw new Error('ZIP内に表示できるファイルが見つかりません。');
    } else {
      const lower = file.name.toLowerCase();
      const rec = { name: file.name, lower, text: null, file, zipEntry: null, blobUrl: null };
      if (isTextLike(lower)) rec.text = decodeTextBuffer(await file.arrayBuffer(), file.name);
      allEntries.set(normalizePath(file.name), rec);
      if (lower.endsWith('.xml')) xmlFiles = [rec];
      else if (lower.endsWith('.xsl') || lower.endsWith('.xslt')) xslFiles = [rec];
      displayFiles = [rec];
    }
    renderFileList();
    renderXslSelect();
...
function renderFileList() {
  ...
  for (const group of groups) {
    ...
    details.open = group.open || items.some(x => x.index === activeDisplayIndex);
    ...
    for (const { entry, index } of items) {
      ...
      b.type = 'button';
      b.className = 'file-item' + (index === activeDisplayIndex ? ' active' : '');
...
async function selectDisplayFile(index) {
  try {
    ...
    if (['TEXT', 'XSL', 'CSS', 'JS'].includes(kind)) return await renderTextFile(entry);
...
async function selectXml(index) {
  try {
    ...
    const doc = parser.parseFromString(stripDoctype(xf.text), 'application/xml');
    if (doc.querySelector('parsererror')) throw new Error('XMLのパースに失敗しました。');
    activeDoc = doc;
    ...
    if (mode !== 'manual') {
      ...
      if (xsl) {
        ...
        throw new Error('ZIP内またはXMLのxml-stylesheet処理命令にXSL/XSLTが見つかりません。');
      }
    }
...
function chooseXslForXml(doc, xmlFile) {
  ...
  if (selected && selected !== '__auto__') return xslFiles.find(x => x.name === selected) || null;
  ...
  const xmlBase = basename(xmlFile.name).replace(/\.xml$/i, '').toLowerCase();
  const sameBase = xslFiles.find(x => basename(x.name).replace(/\.xslt?$/i, '').toLowerCase() === xmlBase);
  if (sameBase) return sameBase;
  const sameDir = xslFiles.find(x => dirname(x.name) === dirname(xmlFile.name));
...
async function renderWithXslt(xmlDoc, xmlFile, xslFile) {
  if (!window.XSLTProcessor) throw new Error('このブラウザはXSLTProcessorに対応していません。Chrome/Edge/Firefox/Safariで試してください。');
  const xslDoc = await loadResolvedXsl(xslFile);
  const processor = new XSLTProcessor();
  processor.importStylesheet(xslDoc);
  const result = processor.transformToDocument(xmlDoc);
  if (!result || result.querySelector?.('parsererror')) throw new Error('XSLT変換結果が不正です。');
  ...
  iframe.addEventListener('load', () => {
    try {
      ...
      const h = Math.max(900, doc.documentElement.scrollHeight, doc.body?.scrollHeight || 0);
  ...
  setStatus(`XSLT iframe表示中: ${xmlFile.name} / stylesheet: ${xslFile.name} / compat: ${compatSelect?.value || 'iewrap'}`);
}
...
function serializeXsltResult(result) {
  ...
  if (result.body && result.body.childNodes.length) {
    return '<html><head><meta charset="utf-8"></head><body>' + result.body.innerHTML + '</body></html>';
  }
  return '<html><head><meta charset="utf-8"></head><body>' + serializer.serializeToString(root) + '</body></html>';
}
...
function applyAnalyzedXslFixes(doc) {
  ...
  for (const table of doc.querySelectorAll('table')) {
    ...
    if (cols.length && !table.style.width && !table.getAttribute('width')) {
      const total = cols.reduce((sum, col) => sum + (explicitHtmlLength(col.getAttribute('width')) || explicitCssLength(col.style.width) || 0), 0);
...
function fixTerritoryNestedColumns(doc) {
  ...
  if (!doc || !doc.body) return;
  for (const outer of doc.querySelectorAll('table.Territory[cellpadding="5"]')) {
    for (const inner of outer.querySelectorAll(':scope > tbody > tr > td > table.Lterritory, :scope > tbody > tr > td > table.Rterritory')) {
      ...
      if (cols.length) {
        const widths = cols.map(c => explicitHtmlLength(c.getAttribute('width')) || explicitCssLength(c.style.width) || 0);
      ...
      for (const pre of inner.querySelectorAll('td.normalTL > pre.normal, pre.normal')) {
        pre.style.width = '100%';
...
function protectNonBreakingNumericTokens(doc) {
  ...
  const tokenRe = /(?:〒\s*)?\d{3}-\d{4}|\d{2,5}-\d{1,4}-\d{3,4}|\d{6,13}|[¥￥]?\d{1,3}(?:,\d{3})+(?:\s*円)?/g;
  ...
  const walker = doc.createTreeWalker(doc.body, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (parent.closest('[data-egov-nowrap-token="1"]')) return NodeFilter.FILTER_REJECT;
      const text = node.nodeValue || '';
      tokenRe.lastIndex = 0;
      return tokenRe.test(text) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
    }
  });
...
function normalizeYenBackslashes(doc) {
  ...
  const walker = doc.createTreeWalker(doc.body, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      const parent = node.parentElement;
      if (!parent || skipTags.has(parent.tagName)) return NodeFilter.FILTER_REJECT;
      return amountBackslashRe.test(node.nodeValue || '') ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
    }
  });
...
function applyPrintScaleOnly(doc, { screenFit = false } = {}) {
  ...
  const contentWidth = Math.max(
    ...
    ...[...doc.querySelectorAll('table')].map(t => t.scrollWidth || t.getBoundingClientRect().width || 0),
    printable
  );
  const scale = Math.min(1, printable / contentWidth);

  doc.documentElement.dataset.egovContentWidth = String(Math.ceil(contentWidth));
...
function normalizeFormLikeTextBlocks(doc) {
  ...
  for (const ta of textareas) {
    ...
    if (!div.style.width && cs.width && cs.width !== 'auto') div.style.width = cs.width;
    if (!div.style.height && cs.height && cs.height !== 'auto') div.style.height = cs.height;
    ta.replaceWith(div);
  }
  ...
  for (const input of inputs) {
    ...
    if (!span.style.width && cs.width && cs.width !== 'auto') span.style.width = cs.width;
    if (!span.style.height && cs.height && cs.height !== 'auto') span.style.height = cs.height;
...
function normalizeXsltTables(doc) {
  ...
  for (const table of tables) {
    ...
    if (width > 0) {
      ...
      const explicit = explicitCssLength(table.style.width) || explicitHtmlLength(table.getAttribute('width'));
      ...
      if (parentCell) {
        ...
        const current = explicitCssLength(parentCell.style.width) || explicitHtmlLength(parentCell.getAttribute('width')) || parseFloat(win.getComputedStyle(parentCell).width || '0') || 0;
        if (need > current + 1) {
  ...
  for (const table of tables.filter(t => !t.parentElement?.closest?.('table'))) {
    const explicit = explicitCssLength(table.style.width) || explicitHtmlLength(table.getAttribute('width'));
    const measured = Number(table.dataset.egovMeasuredWidth || 0);
    const rectWidth = table.getBoundingClientRect().width || 0;
    const target = Math.max(explicit || 0, measured || 0, rectWidth || 0, A4_PX);
    if (target > 0) {
...
function applyA4WrapConstraint(doc) {
  ...
  const outerTables = [...doc.querySelectorAll('table')].filter(t => !t.parentElement?.closest?.('table'));
  ...
  for (const el of [...doc.body.querySelectorAll('div,span,p,font,textarea,input')]) {
...
function relaxClippingAfterWrap(doc) {
  ...
  const win = doc.defaultView;
  if (!win) return;
  for (const el of doc.querySelectorAll('td,th,div,span,p,font,textarea,input')) {
    const cs = win.getComputedStyle(el);
    const h = parseFloat(cs.height || '0') || explicitCssLength(el.style?.height) || 0;
    if (h > 0) el.style.minHeight = `${Math.ceil(h)}px`;
    if (/hidden/i.test(cs.overflowY) || /hidden/i.test(cs.overflow)) el.style.overflowY = 'visible';
  }
}
...
function hardWrapCappedTableText(doc) {
  ...
  for (const el of doc.querySelectorAll('table[data-egov-a4-capped="1"] [nowrap]')) {
  ...
  for (const table of doc.querySelectorAll('table[data-egov-a4-capped="1"]')) {
    candidates.push(...table.querySelectorAll('textarea,input,div,span,p,font,td,th'));
  }
  ...
  for (const el of candidates) {
    ...
    if (!Number.isFinite(width) || width < 12) {
      ...
      if (cell) width = Math.max(12, contentWidth(cell, win.getComputedStyle(cell)));
    }
    ...
    const longestRun = Math.max(...plain.split(/\s|\n/).map(x => [...x].length), 0);
...
function isHardWrapLeaf(el) {
  ...
  const elementChildren = [...el.children].filter(ch => !['BR','WBR'].includes(ch.tagName));
...
function setPrintableText(el, text, cs) {
  if (el.tagName === 'TEXTAREA') {
    ...
    if (['SPAN','FONT'].includes(el.tagName)) el.style.display = 'inline-block';
  }
}
...
function findPrintableWidth(doc, win, fallback) {
  ...
  let best = fallback;
  for (const el of [doc.body, ...doc.querySelectorAll('body > table, body > div')]) {
    const w = explicitCssLength(el.style?.width) || explicitHtmlLength(el.getAttribute?.('width')) || parseFloat(win.getComputedStyle(el).width || '0') || 0;
...
function capTableTree(table, available, win) {
  ...
  const current = table.getBoundingClientRect().width || Number(table.dataset.egovMeasuredWidth || 0) || table.scrollWidth || 0;
  ...
  for (const row of [...table.rows]) {
    ...
    const declared = cells.map(cell => explicitCssLength(cell.style.width) || explicitHtmlLength(cell.getAttribute('width')) || 0);
    const sumDeclared = declared.reduce((a, b) => a + b, 0);
    const equal = cap / cells.reduce((a, c) => a + Math.max(1, Number(c.getAttribute('colspan') || 1)), 0);

    for (let i = 0; i < cells.length; i++) {
      const cell = cells[i];
      const span = Math.max(1, Number(cell.getAttribute('colspan') || 1));
      let cw = declared[i] || equal * span;
      if (sumDeclared > cap && declared[i]) cw = declared[i] * cap / sumDeclared;
      ...
      for (const childTable of [...cell.children].filter(ch => ch.tagName === 'TABLE')) {
        capTableTree(childTable, cw - horizontalExtras(cell, win), win);
      }
      for (const el of [...cell.querySelectorAll('div,span,p,font,textarea,input')]) {
        if (el.closest('table') && el.closest('table') !== table && !cell.contains(el.closest('table'))) continue;
        capTextElementToAvailableWidth(el, cw - horizontalExtras(cell, win), win);
      }
    }
  }
}
...
function capTextElementToAvailableWidth(el, available, win) {
  ...
  const own = explicitCssLength(el.style?.width) || explicitHtmlLength(el.getAttribute?.('width')) || 0;
  ...
  const h = explicitCssLength(el.style?.height) || parseFloat(win.getComputedStyle(el).height || '0') || 0;
  if (h > 0) {
...
function measureNestedTableWidth(table, win) {
  ...
  for (const row of [...table.rows]) {
    let rowWidth = 0;
    for (const cell of [...row.cells]) rowWidth += measureNestedCellWidth(cell, win);
    rowMax = Math.max(rowMax, rowWidth);
  }

  return Math.max(explicit || 0, rowMax || 0, scroll || 0, rect.width || 0, computed || 0);
}

function measureNestedCellWidth(cell, win) {
  const own = explicitCssLength(cell.style.width) || explicitHtmlLength(cell.getAttribute('width'));
  ...
  for (const child of [...cell.children]) {
    if (child.tagName === 'TABLE') {
      inner = Math.max(inner, Number(child.dataset.egovMeasuredWidth || 0), measureNestedTableWidth(child, win));
      continue;
    }
  ...
  if (!cell.children.length && cell.textContent?.trim()) {
    inner = Math.max(inner, cell.scrollWidth || 0, cell.getBoundingClientRect().width || 0);
  }
...
function measureElementWidth(el, win) {
  const own = explicitCssLength(el.style?.width) || explicitHtmlLength(el.getAttribute?.('width'));
  const cs = win.getComputedStyle(el);
  const computed = parseFloat(cs.width || '0') || 0;
  const rect = el.getBoundingClientRect?.();
...
function inferTableColumnWidths(table, win) {
  ...
  for (const row of rows) {
    ...
    for (const cell of [...row.cells]) {
      ...
      if (w > 0) {
        const per = w / span;
        for (let i = 0; i < span; i++) widths[col + i] = Math.max(widths[col + i] || 0, per);
      }
      col += span;
    }
  }
...
function inferCellWidth(cell, win) {
  ...
  if (Number.isFinite(computed) && computed > 8 && cell.children.length === 0) return computed;

  let max = 0;
  for (const child of [...cell.children]) {
    const cstyle = child.style || {};
    const cw = explicitCssLength(cstyle.width) || explicitHtmlLength(child.getAttribute?.('width'));
...
function applyTableWidth(table, colWidths, total, doc) {
  ...
  colgroup.replaceChildren(...colWidths.map(w => {
...
function applyIeLikeWrapEmulation(doc) {
  ...
  const candidates = [...doc.body.querySelectorAll('textarea,input,div,span,p')]
...
function isWrapCandidate(el, win) {
  ...
  if (pos !== 'absolute' && pos !== 'fixed' && tag !== 'TEXTAREA' && tag !== 'INPUT') return false;
...
function computedCanvasFont(cs) {
  ...
  return cs.font && cs.font !== '' ? cs.font : `${cs.fontStyle} ${cs.fontVariant} ${cs.fontWeight} ${cs.fontSize}/${cs.lineHeight} ${cs.fontFamily}`;
}
...
function copyAttributesForPrintBlock(from, to) {
  for (const attr of from.attributes) {
    if (['value', 'type', 'rows', 'cols', 'name'].includes(attr.name.toLowerCase())) continue;
...
async function loadResolvedXsl(xslFile, seen = new Set()) {
  ...
  if (seen.has(key)) throw new Error(`XSL include/importが循環しています: ${xslFile.name}`);
  seen.add(key);
  const parser = new DOMParser();
  const doc = parser.parseFromString(stripDoctype(xslFile.text || ''), 'application/xml');
  if (doc.querySelector('parsererror')) throw new Error(`XSLのパースに失敗: ${xslFile.name}`);

  const nodes = [...doc.querySelectorAll('include, import')].filter(n => /stylesheet|transform/.test(n.parentElement?.localName || '') || n.namespaceURI?.includes('XSL'));
  for (const node of nodes) {
    const href = node.getAttribute('href');
    if (!href) continue;
    const inc = findEntryRelative(href, xslFile.name);
    if (!inc) throw new Error(`XSL参照ファイルがZIP内に見つかりません: ${href}`);
    const incDoc = await loadResolvedXsl(inc, new Set(seen));
    const imported = [...incDoc.documentElement.childNodes].map(ch => doc.importNode(ch, true));
...
async function injectLinkedCss(sourceBody, basePath) {
  const links = [...sourceBody.querySelectorAll?.('link[rel~="stylesheet"][href]') || []];
...
async function rewriteAssetUrls(root, basePath) {
  ...
  for (const el of els) {
    for (const attr of attrs) {
      ...
      if (!value || /^(https?:|data:|blob:|#|mailto:|javascript:)/i.test(value)) continue;
...
function findEntryRelative(href, basePath) {
  ...
  return allEntries.get(clean) || allEntries.get(normalizePath(dirname(basePath) + '/' + clean)) || allEntries.get(normalizePath('/' + clean)) || null;
}
...
function decodeTextBuffer(buffer, name = '') {
  ...
  const m = head.match(/encoding\s*=\s*["']([^"']+)["']/i) || head.match(/charset\s*=\s*["']?([a-z0-9_\-]+)/i);
  if (m) {
  ...
  try {
    ...
    try { return new TextDecoder('shift_jis').decode(bytes); } catch (__) {}
    try { return new TextDecoder('windows-31j').decode(bytes); } catch (__) {}
    return new TextDecoder('utf-8').decode(bytes);
  }
}

function normalizeEncoding(enc) {
  const e = String(enc || '').trim().toLowerCase().replace(/_/g, '-');
  if (['shift-jis', 'shift_jis', 'sjis', 'ms-kanji', 'cp932', 'windows-31j'].includes(e)) return 'shift_jis';
...
async function renderCsv(entry) {
  ...
  viewer.append(table);
  printBtn.disabled = false;
  setStatus(`CSV表示中: ${entry.name}${rows.length >= 5000 ? '（先頭5000行）' : ''}`);
}
...
function ensureUtif() {
  ...
  window.__egovUtifLoading = new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('UTIF.jsの読み込みに失敗しました。TIFF表示にはインターネット接続が必要です。'));
...
async function renderDownloadOnly(entry) {
  ...
  p.textContent = 'この形式は内蔵ビューア対象外です。必要なら保存して外部アプリで開いてください。';
...
function renderFallbackNode(node) {
  ...
  if (/Title$/.test(tag) || /Label$/.test(tag) || tag === 'ArticleCaption') return blockFallback(node, tag === 'ArticleTitle' ? 'article-title' : 'section-title');
  ...
  if (/^(Law|LawBody|MainProvision|Part|Chapter|Section|Subsection|Division|ParagraphSentence|ItemSentence|Column|TableColumn|TableHeaderRow|TableHeaderColumn)$/.test(tag)) {
    const f = document.createDocumentFragment(); appendFallback(f, node); return f;
  }
  return blockFallback(node, 'generic-block');
}
function paragraphFallback(node) {
  const row = el('div', 'paragraph');
  const num = textOfFirst(node, 'ParagraphNum') || (node.getAttribute('Num') || '');
  row.append(el('div', 'paragraph-number' + (num ? '' : ' empty-num'), num || '1'));
  const body = el('div', 'paragraph-body');
  for (const ch of node.childNodes) if (!(ch.nodeType === 1 && ch.localName === 'ParagraphNum')) { const r = renderFallbackNode(ch); if (r) body.append(r); }
...
function itemFallback(node) {
  ...
  for (const ch of node.childNodes) if (ch !== title) { const r = renderFallbackNode(ch); if (r) body.append(r); }
  row.append(body); return row;
}
function tableFallback(node) {
  const table = document.createElement('table'); table.className = 'law-table';
  const rows = [...node.getElementsByTagName('TableRow')];
  for (const rowNode of rows) {
    const tr = document.createElement('tr');
    const cells = [...rowNode.children].filter(c => /Column$/.test(c.localName) || c.localName === 'TableColumn');
    for (const cell of cells) { const td = document.createElement('td'); appendFallback(td, cell); tr.append(td); }
    table.append(tr);
  }
  return rows.length ? table : blockFallback(node, 'generic-block');
}
function rubyFallback(node) { const ruby = document.createElement('ruby'); const base = firstElement(node, 'RubyBase'); const rt = firstElement(node, 'Rt'); if (base) ruby.append(textOf(base)); if (rt) ruby.append(el('rt', '', textOf(rt))); return ruby; }
function inlineFallback(node, tag = 'span', cls = '') { const out = el(tag, cls); appendFallback(out, node); return out; }
function blockFallback(node, cls) { const out = el('div', cls); appendFallback(out, node); return out; }

function buildToc() {
  const heads = [...viewer.querySelectorAll('h1,h2,h3,h4,.part-title,.chapter-title,.section-title,.article-title')]
    .filter(h => h.textContent.trim()).slice(0, 200);
  if (!heads.length) { tocEl.textContent = '目次を生成できませんでした'; tocEl.className = 'toc empty'; return; }
  tocEl.className = 'toc';
  tocEl.replaceChildren(...heads.map((h, i) => {
    if (!h.id) h.id = 'toc-' + i;
    const a = document.createElement('a');
    a.href = '#' + h.id;
    a.textContent = h.textContent.trim().slice(0, 80);
    const tag = h.tagName.toLowerCase();
    a.style.setProperty('--indent', tag === 'h1' ? '0px' : tag === 'h2' ? '10px' : '18px');
    return a;
  }));
}
...
function buildTocFromIframe(doc) {
  const heads = [...doc.querySelectorAll('h1,h2,h3,h4,.part-title,.chapter-title,.section-title,.article-title,[id]')]
    .filter(h => h.textContent.trim()).slice(0, 200);
  if (!heads.length) { tocEl.textContent = 'XSLT表示では目次を生成できませんでした'; tocEl.className = 'toc empty'; return; }
  tocEl.className = 'toc';
  tocEl.replaceChildren(...heads.map((h, i) => {
    if (!h.id) h.id = 'toc-' + i;
    const a = document.createElement('a');
    ...
    a.addEventListener('click', (ev) => {
      ev.preventDefault();
      document.querySelector('#xsltFrame')?.contentDocument?.getElementById(h.id)?.scrollIntoView({ block: 'start' });
    });
    return a;
  }));
}

function firstElement(root, tag) { return [...root.getElementsByTagName(tag)][0] || null; }
function textOfFirst(root, tag) { const n = firstElement(root, tag); return n ? textOf(n) : ''; }
...
function showError(err) { console.error(err); viewer.replaceChildren(); printBtn.disabled = true; statusEl.className = 'status error'; statusEl.textContent = err?.message || String(err); }
function normalizePath(p) { return p.replace(/\\/g, '/').replace(/^\.\//, '').replace(/\/+/g, '/').replace(/^\//, ''); }
function dirname(p) { const n = normalizePath(p); const i = n.lastIndexOf('/'); return i >= 0 ? n.slice(0, i) : ''; }
function basename(p) { const n = normalizePath(p); const i = n.lastIndexOf('/'); return i >= 0 ? n.slice(i + 1) : n; }
function stripDoctype(s) { return s.replace(/<!DOCTYPE[\s\S]*?>/i, ''); }
function escapeAttr(s) { return String(s).replace(/[&\"<>]/g, c => ({'&':'&amp;','\"':'&quot;','<':'&lt;','>':'&gt;'}[c])); }

function decodeHtml(s) { const t = document.createElement('textarea'); t.innerHTML = s; return t.value; }
function resetObjectUrls() { for (const u of objectUrls) URL.revokeObjectURL(u); objectUrls = []; }
